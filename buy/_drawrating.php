...
$query=mysql_query("SELECT total_votes, total_value, used_ips FROM $rating_dbname.$rating_tableName WHERE ItemID='$id' ")or die(" Error: ".mysql_error());


// insert the id in the DB if it doesn't exist already
// see: http://www.masugadesign.com/the-lab/scripts/unobtrusive-ajax-star-rating-bar/#comment-121
if (mysql_num_rows($query) == 0) {
$sql = "INSERT INTO $rating_dbname.$rating_tableName (`ItemID`,`total_votes`, `total_value`, `used_ips`) VALUES ('$id', '0', '0', '')";
...
$voted=mysql_num_rows(mysql_query("SELECT used_ips FROM $rating_dbname.$rating_tableName WHERE used_ips LIKE '%".$ip."%' AND ItemID='".$id."' "));
...
if ($static == 'static') {
      ...
      $rater.= "<label style='font-weight:bold; color:purple;'>Rate this item</label>";
